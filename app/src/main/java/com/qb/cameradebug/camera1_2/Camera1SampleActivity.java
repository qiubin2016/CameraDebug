package com.qb.cameradebug.camera1_2;
...
public class Camera1SampleActivity extends AppCompatActivity implements Handler.Callback {
    ...
    private static final String TAG = "Camera1SampleActivity";
    ...
    public boolean handleMessage(Message msg) {
        switch (msg.what) {
            case MSG_OPEN_CAMERA: {  //打开摄像头
                openCamera(msg.arg1);
                break;
            }
            case MSG_CLOSE_CAMERA: {  //关闭摄像头
                closeCamera();
                break;
            }
            case MSG_SET_PREVIEW_SIZE: {  //设置预览尺寸
                int shortSide = msg.arg1;
                int longSide = msg.arg2;
                setPreviewSize(shortSide, longSide);
                break;
            }
            case MSG_SET_PREVIEW_SURFACE: {  //设置预览surface
    ...
    protected void onResume() {
        ...
        if (!isRequiredPermissionsGranted() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            Log.i(TAG, "onResume begin 1!");
            requestPermissions(REQUIRED_PERMISSIONS, REQUEST_PERMISSIONS_CODE);
        } else if (mCameraHandler != null) {
            Log.i(TAG, "onResume begin 2!");
